<template>
  ...
  >
    <div class="container h-65vh overflow-auto pr-16px">
      <div>
        <el-descriptions class="margin-top" title="（SO₂）仪器精密度审核记录表" :column="2" border>
        ...
        <el-descriptions class="margin-top" :column="1" border>
          ...
          <el-descriptions-item label="审核结果">
            <el-radio v-model="formData.chkRes" label="1">
              合格
            </el-radio>
            <el-radio v-model="formData.chkRes" label="2">
              不合格
            </el-radio>
          </el-descriptions-item>
        </el-descriptions>
...
export default {
  ...
  methods: {
    onGetReportDetail () {
      gasPrecisionDetail('so2', { id: this.reportId }).then(({data}) => {
    ...
    submit() {
      ...
      gasPrecisionUpdate('so2', this.formData).then(res=>{
