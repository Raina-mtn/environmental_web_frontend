<template>
  ...
  >
    <div class="container h-65vh overflow-auto pr-16px">
      <div>
        <el-descriptions class="margin-top" title="（CO）仪器精密度审核记录表" :column="2" border>
...
export default {
  ...
  methods: {
    onGetReportDetail () {
      gasPrecisionDetail('co', { id: this.reportId }).then(({data}) => {
    ...
    submit() {
      ...
      gasPrecisionUpdate('co', this.formData).then(res=>{
