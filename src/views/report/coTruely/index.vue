...
export default {
  ...
  methods:{
    getList(){
      ...
      gasTruelyList('co', {...this.formData,stationId:this.stationId}).then(res=>{
    ...
    handleExport (props) {
      ...
      this.$confirm('确认导出该文件？', '提示', {
        ...
        gasExport('co', {id: props.id}).then(res => {
          // const date = new Date()
          ...
          // downloadFile(res, `${time}co仪器运行状况校准记录`, 'application/vnd.ms-excel')
          ...
          if ('download' in document.createElement('a')) { // 支持a标签download的浏览器
            ...
            link.download = time + '-co仪器运行状况校准记录'
